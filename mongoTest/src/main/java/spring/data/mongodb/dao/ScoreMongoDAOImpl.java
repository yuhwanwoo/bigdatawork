...
public class ScoreMongoDAOImpl implements ScoreMongoDAO {
	...
	public List<ScoreDTO> findCriteria(String key, String value) {
		...
		/*
		 * Query클래스의 addCriteria메소드와 Criteria클래스의 where메소드의 활용
		 * -------------------------------------
	...
	public ScoreDTO findById(String key, String value) {
		//1. 조건을 객체로 작성
		Criteria criteria=new Criteria(key);//조건에 대한 처리를 하는 클래스
	...
	public void update(ScoreDTO document) {
		...
		mongoTemplate.updateMulti(query, update, "score"); //score는 db안의 collection이름인거 알지?
	}
	...
	public List<ScoreDTO> findAll(int pageNo) {
		//PagingAndSortingRepositoy에서 제공하는 findAll을
		//호출하면 spring-data내부에서 페이징 처리가 된 객체를 전달 받을 수 있다.
		//페이징 처리를 내부에서 할 수 있도록 필요한 정도를
		//findAll메소드에서 Pageable타입의 매개변수로 전달받는다.
		//Pageable을 구현하고 있는 PageRequest객체를 넘겨준다.
		//					  ---------
		//			현재 페이지번호(page), size(화면에 표시할 게시글 개수)
		//scoreRepository.findAll(new PageRequest(page, size))
		//
		//[리턴]
